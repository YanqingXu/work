...
static const size_t MSG_HEADER_SIZE		= 6;	//消息头长度
...
bool CSProtocol::serialize(Message *message, ByteStream &stream)
{
	if (message == NULL)
	{
		return false;
	...
	stream << (short)message->getLength();		//消息长度(消息头和消息体的总长度)
...
bool CSProtocol::unserialize(ByteStream &stream, Message *&message)
{
	message = NULL;
	...
	short msgid = 0;	//消息标识
	short msglen = 0;	//消息长度(消息头和消息体的总长度)
